...
public class Ex4_Casting {
	...
	public static void main(String[] args) {
		 ...
		 *  - 계산을 위해 특정값을 일시적으로 변환하는 경우( / 연산자 설명할 때 다시 언급함)
		 */
		...
		double num3_1 = 1 / 2;
		double num3_2 = (double)1 / 2; // 1/2는 0.5가 아니라 0
		System.out.println(num3_1); // 1.0/2는 0.5
		System.out.println(num3_2);
		
		//자동타입변환 예: 연산과정
		int num4_1 = 1;
		double num4_2 = 1.23;
		//num4_1과 더하는 값이 실수이기 때문에 num4_1을 실수로 자동타입변환을 한 후, 실수+실수를 계싼해 num4_3에 저장
		//더할 때 자동 타입변화를 함
		double num4_3 = num4_1 + num4_2;
		//num4_1과 더하는 값이 정수이기 때문에 정수+정수를 한 후 더한 값을 실수로 자동 타입변환을 해 num4_3에 저장
		//더한 후에 저장하기 전에 타입변환을 함
		double num4_4 = num4_1 + 5;
		System.out.println(num4_3);
		System.out.println(num4_4);
		
		//p.55~56 정수를 실수로 자동변환할 때 정밀도가 떨어질 수 있음 int-> float
		//int -> float으로 타입변환하기보다 double로 타입변환해 활용함
